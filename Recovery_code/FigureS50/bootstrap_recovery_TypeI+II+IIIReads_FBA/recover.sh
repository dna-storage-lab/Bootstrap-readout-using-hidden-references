...
# Program Parameters
# ----------------------------------------------------------
WatermarkSeq="./configure/SequenceLengthALL_FILER093"
PayloadSize=20000
NumThreads=1 
MaxThreads=1
Job_Count=0
...
# Input Files 
# ----------------------------------------------------------
InputFastq="../../../Sequencing_data/ART_simulated_data/err_0.012/DNA-20Kb-RB-Sim-2.fastq"           
ReadLength=150
CorrThreshold=96
NumReads=665  # 5x
...
(
    ...
    ./R0.93_bootstrap_recovery_step1.sh \
        "$SubReadsFile" \
        "$WatermarkSeq" \
        "$ExpDir_S1" \
        "$ReadLength" \
    ...
    if [ "$err1" -eq 0 ]; then
        ...
        ./R0.93_bootstrap_recovery_step2.sh \
            "$WatermarkSeq" \
            "$ExpDir_S2" \
            "$ReadLength" \
            "$ins" \
        ...
        if [ "$err2" -eq 0 ]; then
            ...
            ./R0.93_bootstrap_recovery_step3.sh \
                "$SubReadsFile" \
                "$WatermarkSeq" \
                "$ExpDir_S3" \
                "$ReadLength" \
