...
int process_r083(const char* input_file, std::string &out_seq) {
    ...
    {
        std::ifstream fperm("./configure/permutation64512", std::ios::binary);
        if (!fperm) {
    ...
    int* watermark1;
    watermark1 = (int*)malloc((80640 + 81000) * sizeof(int));

    FILE* watermarkFP;    
    if ((watermarkFP = fopen("./configure/sequenceL80640NoPeriod2FILE", "rb")) == NULL)
    {
        printf("Can't open the sequenceL80640NoPeriod for convert\n");
        exit(0);
    }
    fread(&watermark1[0], sizeof(int), (80640 + 81000), watermarkFP); 
    fclose(watermarkFP);
    

    std::vector<int> temp(80640);
    for (int j = 0; j < 40320; j++)
    {
        temp[j] = watermark1[j];
    }                
    for (int j = 40320; j < 80640; j++)
    {
        temp[j] = watermark1[j + 40500];
    }

    std::vector<int> watermark(temp.begin(), temp.end());
    free(watermark1);
...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::fprintf(stderr, "Usage: %s dec_results.txt stage2_residual_reads.txt output.txt\n", argv[0]);
        return 1;
    }
